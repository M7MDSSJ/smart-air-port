...
export class UserManagementService {
  ...
  async verifyEmail(token: string): Promise<VerifyEmailResponseDto> {
    return this.userRepository.withTransaction(async (session) => {
      const user = await this.userRepository.findByToken(token, { session });
    ...
    return this.userRepository.withTransaction(async (session) => {
      ...
      if (updateProfileDto.phoneNumber) {
        if (updateProfileDto.phoneNumber !== user.phoneNumber) {
          const phoneExists = await this.userRepository.findByPhoneNumber(updateProfileDto.phoneNumber, { session });
      ...
      if (updateProfileDto.email && updateProfileDto.email !== user.email) {
        const existingUser = await this.userRepository.findByEmail(updateProfileDto.email, { session });
        if (existingUser) {
