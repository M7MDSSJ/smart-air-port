...
export class EmailService implements OnModuleInit {
  ...
  private async verifyTransporter():Promise<void> {
    try {
      ...
      if(success) this.logger.log('Email transporter verified successfully');
    } catch (error) {
      this.logger.error('Failed to verify email transporter', (error as Error).stack);
  ...
  async sendVerificationEmail(email: string, code: string):Promise<void> {
    ...
    const html = this.generateEmailTemplate({
      ...
      buttonText: '',
      buttonUrl: '',
      footer: 'If you didn’t request this, feel free to ignore this email.<br><br>Best,<br>The Airport Team',
    });
  ...
  private async sendEmail(options: { to: string; subject: string; html: string; from: string; }): Promise<void> {
    try {
      await this.transporter.sendMail(options);
      this.logger.log(`Email sent successfully to ${options.to} with subject: ${options.subject}`);
    } catch (error: any) {
