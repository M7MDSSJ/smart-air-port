...
import { QueryFlightDto, SortBy, SortOrder, TripType, DepartureTimeRange } from './dto/query-flight.dto';
...
export class FlightSearchService {
  ...
  async searchAvailableFlights(query: QueryFlightDto): Promise<{ paginatedFlights: FormattedFlight[]; total: number }> {
    ...
    if (!departureAirport || !arrivalAirport || !departureDate || !tripType || !adults || !cabinClass) {
      throw new HttpException(await this.i18n.t('errors.missingRequiredFields', { lang: language }), HttpStatus.BAD_REQUEST);
    }
    if (tripType === TripType.RoundTrip && !returnDate) {
      throw new HttpException(await this.i18n.t('errors.returnDateRequired', { lang: language }), HttpStatus.BAD_REQUEST);
    }
    if (tripType === TripType.MultiCity && (!multiCityLegs || multiCityLegs.length === 0)) {
      throw new HttpException(await this.i18n.t('errors.multiCityLegsRequired', { lang: language }), HttpStatus.BAD_REQUEST);
    }

    const cacheKey = this.generateCacheKey(query);
    const cachedResult = await this.cacheService.get<{ paginatedFlights: FormattedFlight[]; total: number }>(cacheKey);
    if (cachedResult) {
    ...
    try {
      if (tripType === TripType.OneWay) {
        ...
        rawFlights = await this.amadeusService.searchMultiCityFlights(
          multiCityLegs!.map(leg => ({ origin: leg.departureAirport, destination: leg.arrivalAirport, departureDate: leg.departureDate })),
          adults,
          children,
          infants,
          cabinClass,
      ...
      throw new HttpException(await this.i18n.t('errors.amadeusFetchFailed', { lang: language }), HttpStatus.INTERNAL_SERVER_ERROR);
    }

    if (rawFlights.length === 0) return { paginatedFlights: [], total: 0 };

    // Format and filter flights
    const formattedFlights = await this.formattingService.formatFlightResponse(rawFlights, query);
    let filteredFlights = formattedFlights.filter(flight => {
      ...
      if (departureTimeRange) {
        ...
        switch (departureTimeRange) {
          case DepartureTimeRange.Morning: return hour >= 6 && hour < 12;
          case DepartureTimeRange.Afternoon: return hour >= 12 && hour < 18;
          case DepartureTimeRange.Evening: return hour >= 18 && hour < 21;
          case DepartureTimeRange.Night: return hour >= 21 || hour < 6;
        }
      }
      return true;
    });
    ...
    filteredFlights.sort((a, b) => {
      ...
      else if (effectiveSortBy === SortBy.Duration) comparison = a.durationInMinutes - b.durationInMinutes;
      else if (effectiveSortBy === SortBy.Stops) comparison = a.numberOfStops - b.numberOfStops;
      else if (effectiveSortBy === 'totalPrice') comparison = a.totalPrice - b.totalPrice;
      return effectiveSortOrder === 'desc' ? -comparison : comparison;
    });

    // Paginate
    const startIndex = (page - 1) * limit;
    const paginatedFlights = filteredFlights.slice(startIndex, startIndex + limit);
